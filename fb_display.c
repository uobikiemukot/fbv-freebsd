...
video_color_palette_t map332 = {0, 256, red, green, blue, NULL};
u_char red_b[256], green_b[256], blue_b[256];
video_color_palette_t map_back = {0, 256, red_b, green_b, blue_b, NULL};


int openFB(const char *name);
void closeFB(int fh);
void getVideoAdapterInfo(int fh, video_adapter_info_t *ainfo);
void setVideoMode(int fh, int mode);
void getVideoInfo(int fh, video_info_t *vinfo);
...
void fb_display(unsigned char *rgbbuff, unsigned char * alpha, int x_size, int y_size, int x_pan, int y_pan, int x_offs, int y_offs)
{
    ...
    getVideoAdapterInfo(fh, &ainfo);
    getVideoInfo(fh, &vinfo);
    
    x_stride = (ainfo.va_line_width * 8) / vinfo.vi_depth;
   
    /* correct panning */
    if(x_pan > x_size - x_stride) x_pan = 0;
    if(y_pan > y_size - vinfo.vi_height) y_pan = 0;
    /* correct offset */
    if(x_offs + x_size > x_stride) x_offs = 0;
    if(y_offs + y_size > vinfo.vi_height) y_offs = 0;
    
    /* blit buffer 2 fb */
    fbbuff = convertRGB2FB(fh, rgbbuff, x_size * y_size, vinfo.vi_depth, &bp);
    blit2FB(fh, fbbuff, alpha, x_size, y_size, x_stride, vinfo.vi_height, x_pan, y_pan, x_offs, y_offs, bp);
    free(fbbuff);
...
void getVideoAdapterInfo(int fh, video_adapter_info_t *ainfo)
{
    if (ioctl(fh, FBIO_ADPINFO, ainfo)){
...
void setVideoMode(int fh, int mode)
{
    if (ioctl(fh, FBIO_SETMODE, mode)){
        fprintf(stderr, "ioctl FBIO_SETMODE: %s\n", strerror(errno));
        exit(1);
    }
}

void getVideoMode(int fh, int *mode)
{
    if (ioctl(fh, FBIO_GETMODE, mode) < 0) {
        fprintf(stderr, "ioctl FBIO_GETMODE: %s\n", strerror(errno));
        exit(1);
    }
}
...
void getVideoInfo(int fh, video_info_t *vinfo)
{
    ...
    getVideoMode(fh, &video_mode);

    vinfo->vi_mode = video_mode;
    if (ioctl(fh, FBIO_MODEINFO, vinfo)){
        fprintf(stderr, "ioctl FBIOGET_FSCREENINFO: %s\n", strerror(errno));
        exit(1);
    }
}

void make332map(video_color_palette_t *map)
{
    int rs, gs, bs, i;
    int r = 8, g = 8, b = 4;
...
void set8map(int fh, video_color_palette_t *map)
{
...
void get8map(int fh, video_color_palette_t *map)
{
